...
func NewDkgSelfService(config *types2.DKGConfig, pm types.PeerManager, hostClient host.Host, hash string, storeDB types2.StoreDB) (*Dkg, error) {
	s := &Dkg{
		config:  config,
		pm:      pm,
		storeDB: storeDB,
...
func (p *DkgSelf) GetResult() (*dkg.Result, error) {
	return p.dkg.GetResult()
}

func (p *DkgSelf) Handle(s network.Stream) {
	data := &dkg.Message{}
...
func (p *DkgSelf) closeDone() {
	close(p.done)
	p.hostClient.RemoveStreamHandler(peer.GetProtocol(p.hash))
}

func (p *DkgSelf) OnStateChanged(oldState types.MainState, newState types.MainState) {
